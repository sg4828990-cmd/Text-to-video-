...
async def start(client: Client, msg: Message):
    ...
    await start_message.edit_text(
        Data.START.format(msg.from_user.mention) +
        "Checking status Ok... Command Nhi Bataunga **Bot Made BY 𝒮𝒶𝒽𝒾𝓁™👨🏻‍💻**🔍\n\n"
        "Progress:[🟩🟩🟩🟩🟩🟩🟩🟩🟩] 100%\n\n"
    )
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            try:  
                ...
                cc = f'**🎞️ VID_ID: {str(count).zfill(3)}.\n\n📝 Title: {name1} {𝒮𝒶𝒽𝒾𝓁} {res}.mkv\n\n📚 Batch Name: {b_name}\n\n📥 𝐸𝓍𝓉𝓇𝒶𝒸𝓉𝑒𝒹 𝒷𝓎 : {CR}\n\n**━━━━━✦{𝒮𝒶𝒽𝒾𝓁}✦━━━━━**'
                cc1 = f'**📁 PDF_ID: {str(count).zfill(3)}.\n\n📝 Title: {name1} {𝒮𝒶𝒽𝒾𝓁}.pdf\n\n📚 Batch Name: {b_name}\n\n📥 𝐸𝓍𝓉𝓇𝒶𝒸𝓉𝑒𝒹 𝒷𝓎 : {CR}\n\n**━━━━━✦{𝒮𝒶𝒽𝒾𝓁}✦━━━━━**'
                cyt = f'**🎞️ VID_ID: {str(count).zfill(3)}.\n\n📝 Title: {name1} {𝒮𝒶𝒽𝒾𝓁}.mkv\n\n📚 Batch Name: {b_name}\n\n**🔗 𝐕𝐢𝐝𝐞𝐨 𝐥𝐢𝐧𝐤 - ({url})**\n\n📥 𝐸𝓍𝓉𝓇𝒶𝒸𝓉𝑒𝒹 𝒷𝓎 : {CR}\n\n**━━━━━✦{𝒮𝒶𝒽𝒾𝓁}✦━━━━━**'
...
@bot.on_message(filters.command(["𝒮𝒶𝒽𝒾𝓁"]))
async def txt_handler(bot: Client, m: Message):
    editable = await m.reply_text(f"**🔹Send me the TXT file and wait.**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
